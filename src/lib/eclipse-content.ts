...
export function composeEclipse(event: CalendarEventInput, chart: ChartData): LunationReading {
  ...
  const primer: ReadingSection[] = [
    {
      heading: "an eclipse is not just a big moon",
      body: `A new moon and a full moon happen every month. An eclipse only happens when that moon lines up almost exactly with the lunar nodes, the two points where the moon's path crosses the sun's. That alignment is why an eclipse carries so much more force: it is a lunation landing on the chart's fated axis, the line that describes where you are growing and what you are leaving behind. Eclipses tend to move things that ordinary moons only illuminate.`,
    },
    {
