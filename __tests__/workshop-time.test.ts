...
it("is Tuesday 28 July 2026 at 19:00 LA time", () => {
  ...
  expect(la).toContain("Tuesday");
  expect(la).toContain("July 28");
  expect(la).toContain("7:00");
});

it("copy label matches the real instant", () => {
  expect(w.meta).toBe("28 july · 7pm la time · first live class");
});
